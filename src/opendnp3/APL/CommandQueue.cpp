...
namespace apl
{
	...
	/*int CommandQueue::AcceptCommandQ(const apl::Setpoint& arType, size_t aIndex, int aSequence, IResponseAcceptor* apRspAcceptor)
	{
		AcceptCommand<apl::Setpoint>(arType, aIndex, mSetpointQueue, aSequence, apRspAcceptor);
		return mSetpointQueue.size();
	}*/
	int CommandQueue::Size(bool setPoint)
	{
		if (setPoint) {
			return mSetpointQueue.size();
		}
		else {
			return mBinaryQueue.size();
		}
	}
	int CommandQueue::SetSize(int QueueSize)
	{
		MaxSize = QueueSize;
		return MaxSize;
	}
